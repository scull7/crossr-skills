...
- The agent replaces primitives, sentinels, and boolean flags with newtypes, `Option<T>`, and exhaustive match; parses wire values at the edge into newtypes (no naked `String` / `&str` in the core); and uses a builder for complex private-field construction (RD-06).
